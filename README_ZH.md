...
无人值守发送保持窄边界：只有私聊中 `riskLevel=low`、置信度不低于 0.95、无需追问且不包含工作请求的普通回复可以自动批准。群聊、承诺、计划和中高风险内容仍需审阅；人工接管检查和发送回执核对不会关闭。
...
低风险事实只有完成 Markdown 写入、gbrain 精确回读和 PostgreSQL 投影后才可使用。冲突保持隔离；凭据、PII、敏感人物材料和机密候选直接拒绝。
