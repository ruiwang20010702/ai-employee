...
export function adminBaseUrl(value = process.env.AI_EMPLOYEE_ADMIN_URL) {
  ...
  if (
    ...
    throw stableError("AI employee admin URL must be a local loopback HTTP origin");
  }
  return url.origin;
}
...
async function keychainReadToken() {
  if (process.platform !== "darwin") {
    throw stableError("AI employee read credential is unavailable on this platform");
  }
  try {
    const { stdout } = await execFileAsync(
      "/usr/bin/security",
      ["find-generic-password", "-s", "ai-employee-production", "-a", "admin-read-token", "-w"],
      {
        timeout: 10_000,
        maxBuffer: 64 * 1024,
    ...
    throw stableError("AI employee read credential could not be loaded");
  }
}
  ...
  return async function readAdmin(path) {
    ...
    if (!response.ok) {
      throw stableError(response.status === 401
        ? "AI employee read credential was rejected"
        : "AI employee admin request failed");
    }
    try {
      return await response.json();
    } catch {
      throw stableError("AI employee admin returned invalid data");
    }
  };
}
...
const toolDefinitions = Object.freeze([
  { name: "get_status", title: "查看 AI 员工状态", description: "读取本机 AI 员工当前健康、暂停状态和待处理数量。", inputSchema: { type: "object", properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: false } },
  { name: "show_status_panel", title: "显示 AI 员工状态卡片", description: "读取当前状态并以内嵌卡片呈现；宿主不支持组件时仍返回结构化结果。", inputSchema: { type: "object", properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: false }, _meta: { ui: { resourceUri: statusResourceUri }, "openai/outputTemplate": statusResourceUri } },
  ...
  { name: "open_admin_console", title: "打开 AI 员工管理台", description: "返回本机完整管理台入口，用于人工审批、暂停和深度核对。", inputSchema: { type: "object", properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: false } },
]);
...
export function createMcpHandler({ readAdmin = createAdminReader() } = {}) {
  return async function handle(message) {
    ...
    if (message.method === "resources/list") {
      return { resources: [{ uri: statusResourceUri, name: "AI 员工状态卡片", mimeType: "text/html;profile=mcp-app" }] };
    }
    ...
    if (message.method === "tools/call") {
      ...
      if (name === "get_status" || name === "show_status_panel") {
        ...
        return toolResult(result, result.ready ? "AI 员工当前就绪。" : "AI 员工当前需要处理，请检查返回状态。");
      }
      ...
      if (name === "open_admin_console") {
        return {
          ...
          content: [
            ...
            { type: "resource_link", uri: `${adminBaseUrl()}/`, name: "打开 AI 员工管理台", mimeType: "text/html" },
          ],
        };
      }
...
export async function runStdioServer({ input = process.stdin, output = process.stdout, handler = createMcpHandler() } = {}) {
  ...
  for await (const line of lines) {
    ...
    try {
      ...
      output.write(`${JSON.stringify({ jsonrpc: "2.0", id: message.id, error: { code: Number.isInteger(error.code) ? error.code : -32000, message: error.message || "AI employee request failed" } })}\n`);
    }
  }
}
