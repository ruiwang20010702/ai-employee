...
if (lines.length > 100_000) throw new Error("Hermes shadow ledger is too large");
...
if (!result.valid || !result.receipt) {
  throw new Error(`Hermes shadow acceptance is incomplete: ${result.missing.join(",")}`);
}
const target = resolve(output);
const parentMetadata = await lstat(dirname(target));
if (!parentMetadata.isDirectory() || parentMetadata.isSymbolicLink()) {
  throw new Error("Hermes shadow acceptance output parent is invalid");
}
