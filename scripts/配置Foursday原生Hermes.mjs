...
if (args.some((argument, index) =>
  ...
  throw new Error("Usage: 配置Foursday原生Hermes.mjs [--apply] [--replace] [--cron] [--registry /absolute/path.json]");
}
...
const productionConfigPath = defaultProductionConfigPath();
const productionValues = JSON.parse(await readFile(productionConfigPath, "utf8"));
const rawDwsPath = String(productionValues.DWS_PATH ?? "dws");
...
const result = await configureFoursdayNativeProfile({
  ...
  projectRegistryPath: projectRegistryPath ?? fileURLToPath(
    new URL("../hermes/projects.example.json", import.meta.url),
  ),
