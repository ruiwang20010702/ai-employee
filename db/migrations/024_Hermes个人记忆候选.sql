CREATE TABLE hermes_memory_candidates (
...
CREATE TABLE hermes_memory_candidate_sources (
  tenant_id TEXT NOT NULL,
  ...
  FOREIGN KEY (tenant_id, candidate_id)
    REFERENCES hermes_memory_candidates(tenant_id, id)
    ON DELETE CASCADE
);

CREATE INDEX hermes_memory_candidates_queue_idx
ON hermes_memory_candidates (tenant_id, status, updated_at)
WHERE status IN ('proposed', 'retry', 'processing');

CREATE INDEX hermes_memory_candidates_project_idx
ON hermes_memory_candidates (tenant_id, project_id, updated_at DESC);

CREATE INDEX hermes_memory_candidates_principal_idx
ON hermes_memory_candidate_sources (tenant_id, source_principal_key, created_at DESC);
