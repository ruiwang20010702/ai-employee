...
export async function proposeWorkPlanForTask({
  ...
  const result = await runCodexArtifact({
    ...
    prompt: [
      ...
      "图依赖规则：repository_activity_read 只能读取指定日期与 UTC 偏移内的 Git 活动。research、document_draft 或 code_patch 如需使用更早的 repository_activity_read、research 或 document_draft 产物，必须在 inputs.evidenceStepIds 显式引用；步骤排列在后面不等于自动获得前序内容，不得依赖隐式顺序传递。",
