...
项目接入、5 个配方、驾驶舱、时间返还、主动 Worker、会议闭环、GitHub Draft PR、受治理工作图、历史项目导入和固定来源项目记忆同步已随生产提交 `4453246d68ae13a4f4f046a8babe6c5edf69b021` 部署。第 021 号迁移只增加加密的图投影表与索引，图仍是可重放的解释层，不能授权、扣预算、批准或执行。当前生产能力仍只有 `draft_reply`，项目授权为 0，自动发送、计划执行、`proactive_work` 和项目记忆自动同步均保持关闭。
