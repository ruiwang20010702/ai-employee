...
成功激活也仍保持暂停。先回验 `current`、18/18 迁移、9 个 LaunchAgent、未知发送为 0、无活动任务计划，并确认自动发送和计划执行仍关闭；全部通过后才单独执行目标版本的 `control -- resume`。既有死亡任务不在维护发布中重试、取消或关闭。
