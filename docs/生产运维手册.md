...
实际接入人员从仓库运行 `npm run setup:check`；通过 tarball、GitHub 或 npm 安装时，应固定到已经审核的完整提交编号，在自己的空白工作目录安装后运行 `npx --no-install ai-employee check`，不能用会漂移的分支名作为生产版本。它只检查本机依赖、配置权限、必填项和危险能力开关，不连接外部服务，也不回显配置值。没有配置时运行 `npm run setup:init` 或 `npx --no-install ai-employee init` 查看计划；只有增加 `--apply` 才创建当前工作目录下的 `.runtime/production.json`，且拒绝覆盖。再用 `ai-employee secrets` 预览、`ai-employee secrets --apply` 显式写入钥匙串。向导显示“可以预检”后，仍须执行正式的联网只读 `production:preflight`。
