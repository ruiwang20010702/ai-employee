...
1. 先以 `07383a216d9b17ac0f9246dc4c8c728ac3012942` 运行一次“生产发布”。该提交对应专用的 14 迁移兼容基线，包含统一格式的只读迁移状态，不产生 015、016 结构变更；成功后服务进入不可变版本目录。
2. 回读生产健康和 8 个原有服务后，再以当前 `main` 运行第二次“生产发布”。此时工作流已有可验证上一版本，才允许备份并应用 015、016；如新服务切换失败，恢复第一步的服务版本。

这两次都需要 `production` Environment 人工批准。当前主机已在 macOS 登录会话中完成两步现场发布：先固定旧 14 迁移版本，再加密备份并应用 015、016，最后切换到 9 个服务的当前版本。现场还执行了“新结构上恢复旧服务，再恢复当前服务”的真实回退演练；数据库保持只向前，未执行反向迁移。

当前主机的 GitHub Runner 以 LaunchAgent 后台运行时无法访问登录钥匙串，系统 `security` 命令会在只读预检阶段失败。该 Runner 已停用，不能把 GitHub 工作流显示为可用的生产发布通道。现阶段受支持的生产执行位置是已登录的 Codex/macOS 用户会话；每次仍须先完成提交级 CI、人工批准、加密备份和只读预检。若未来恢复 Runner，必须先证明 5 项钥匙串引用在 Runner 会话中都可回读，且不得把密钥复制为 Actions 明文。

常驻服务安装器自身也执行精确回退。首次安装失败时会删除本次新增的 plist，并确认对应任务已经不再加载；升级失败时恢复旧 plist、重新加载并确认任务重新注册。版本切换后还会按明确的 9 项应用服务白名单对账，卸载并备份目标版本已经不存在的服务；数据库 LaunchAgent 不在该白名单内，不会被版本回退清理。任何恢复或状态复核失败都会明确报告“回退不完整”，需要人工核对，不能继续切换 `current`。
...
| 验收项 | 自动证据 | 仍需现场证据 |
|---|---|---|
| PostgreSQL 迁移和并发租约 | GitHub PostgreSQL 16 集成测试；当前迁移后运行 14 迁移旧版本测试 | 当前主机已完成生产 16/16 迁移、两步版本化发布与现场服务恢复；新环境仍需复验 |
