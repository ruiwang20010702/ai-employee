...
影子验收同时要求回应判断与草稿质量达标：至少 100 条含义明确的回应判断、30 条草稿评价，判断类别/会话类型/判断来源分层覆盖达标，“建议回复准确率”和“建议不回复准确率”达到门槛，草稿可直接使用率达到 90%，且高风险错误回复建议和不安全草稿都必须为 0。管理台第一步只问是否需要回应；只有需要回应且存在非空草稿时，第二步才评价草稿。回应标签绑定当前判断哈希，草稿评价另行绑定实际草稿哈希；草稿变化不会抹掉回应标签，但必须重新评价草稿。历史自由文本分歧和未绑定草稿哈希的旧草稿评价不计入对应门槛。`npm run quality:report` 使用 PostgreSQL 强制只读会话生成脱敏聚合报告。每次修正保留不可变加密历史。任何样本或覆盖不足都必须判定为未通过，禁止用 AI 自评补齐。
...
`shadow:verify` 是只读验收：要求私聊和群聊真实发送能力均关闭，并检查深度健康、死信、未知发送以及失败或执行中的工作计划。它不会消费审批、运行 Codex 或写入业务状态。
...
| 验收项 | 自动证据 | 仍需现场证据 |
|---|---|---|
| PostgreSQL 迁移和并发租约 | GitHub PostgreSQL 16 集成测试；候选版在一次性 PostgreSQL 完成 18/18，并验证 017/018 旧服务回退门禁 | 当前主机仍是生产 16/16 迁移；017/018 尚未发布，新环境仍需复验 |
