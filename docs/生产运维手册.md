...
项目接入、5 个配方、驾驶舱、时间返还、主动 Worker、会议闭环、GitHub Draft PR、受治理工作图、历史项目导入、固定来源项目记忆同步和本周工作返还队列已随生产提交 `ca43e02d8e6790404cdccfb9d007c02f890e29b7` 部署。第 021 号迁移只增加加密的图投影表与索引，图仍是可重放的解释层，不能授权、扣预算、批准或执行。当前生产能力仍只有 `draft_reply`；已创建 1 个最小权限 Foursday 项目和 2 条待审记忆候选，自动发送、计划执行、`proactive_work` 和项目记忆自动同步均保持关闭。
