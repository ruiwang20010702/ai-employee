...
| 本机管理台 | 查看任务、计划、记忆和健康，执行受限审批 | 命令行控制仍可使用 |
...
影子验收还要求：真实人工标注样本数达到门槛、判断类别/会话类型/判断来源分层覆盖达标、“不回复准确率”达到门槛、高风险错误回复建议为 0。管理台“判断质量”交替提供风险优先和覆盖抽样，显示完整消息、剩余数量和各层进度；`npm run quality:report` 可只读生成全局质量报告。人工标签绑定当前判断哈希，判断变化后旧标签自动失效；每次修正保留不可变加密历史。样本不足或覆盖偏斜必须判定为未通过，禁止用 AI 自评补齐样本。
...
`production:doctor` 和 `production:preflight` 都是只读检查，不执行迁移；除了文件存在性，还会核对 DWS 当前授权，并运行 Codex 的脱敏诊断，验证登录、配置和网络运行状态。生产预检失败时不要绕过；先修复提示的问题，再重新执行。只有 `db:migrate` 会修改数据库结构，因此检查结果确认无误后再单独运行。
...
工作流不会把 LaunchAgent 指向 Runner 的临时工作区，也拒绝发布尚未进入 `main` 历史的提交。每次发布会以实际检出的提交哈希创建不可变版本目录；只读预检、备份、迁移、服务重装和生产验证全部通过后，才原子更新 `current` 符号链接。安装或验证失败时，工作流会尝试重装上一版本。数据库不会被自动恢复；迁移必须保持向后兼容，涉及破坏性模式变更时必须另行制定专项回滚方案。
...
这是不可恢复操作。确认值绑定当前记录编号、状态和更新时间；预览后产生或变化的相关数据会让执行失败，必须重新预览。活动任务、未知发送、活动计划、待归档消息或仍生效的联系人/项目暂停都会阻止整批删除，应先按正常流程完成、取消、确认发送结果或恢复暂停。项目范围只删除数据库内项目计划、项目记忆及计划明确绑定的来源任务，不删除项目能力清单、代码目录、钉钉文档或其他外部数据。生产执行前还要取得针对具体范围的单独授权；部署迁移 016 本身不构成删除授权。
...
暂停值和原因在数据库中加密保存。联系人暂停会延后草稿和已批准发送，不增加失败次数；项目或能力暂停会阻止新计划开始。正在执行的外部副作用不会被强杀，完成回读或回滚后在步骤边界等待恢复。管理台“局部暂停”栏目提供同样操作，并限制为已配置联系人、已登记项目和已知能力。
