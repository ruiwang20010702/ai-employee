...
- 外部、跨项目、秘密或不可逆工作：普通 Agent Loop 永远不能自行取得权限。
...
| 边界 | 信任等级 | 约束 |
|---|---|---|
| DWS Sidecar | 宿主可信 | 只接收窄协议，持有钉钉能力但不向项目命令传递凭据 |
| 项目注册表 | 用户可信配置 | 私有普通文件、最小字段、绝对目录、无凭据 remote |
...
| Foursday MCP | 最小可信工具 | 只创建证据绑定的记忆候选 |
...
- 未在 allowlist 中的消息不调用模型、不读取项目、不回复。
...
当前版本只允许新建受控 Codex Thread，拒绝未绑定 `thread/resume/fork`。未来恢复必须持久绑定 tenant、platform、principal、conversation、project、thread 和 permission version，并在每次恢复前重新校验。
...
- dynamic tools / selected capabilities；
- base/developer instructions；
- collaboration mode 中的 developer instructions。

随后写入固定 `foursday-workspace + untrusted`，并在 Thread 创建时注入只读、同用户所有的 Foursday Profile 与项目工作说明。
...
同账号在同一会话的真实人工消息会触发 interrupt。AI 标签、既有发送 ID 和同次自动正文不算人工。接管后丢弃过期结果，不能补发旧回复。
...
| Allowlist 用户可请求较多普通工作 | 高风险出口独立、项目注册表限制 | 企业治理插件可增加每联系人项目范围，但不进入个人默认路径 |
| 当前不恢复旧 Codex Thread | resume/fork 失败关闭 | P1 建立持久线程身份账本 |
