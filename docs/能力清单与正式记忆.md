...
gbrain 来源记忆必须使用 `project` 或 `knowledge` 类型并绑定项目。确认前系统实时校验项目仍开放 `knowledge_read`、授权未过期、精确 slug 仍在项目前缀内、页面可读且版本一致；通过后只签发不超过项目授权期限的短期访问租约。`com.ai-employee.memory-source` 每 5 分钟复核，检索自身也拒绝过期租约。项目撤权或路径越界在下一次复核时撤销访问，网络故障只暂停使用，权限恢复后可以续租；页面版本变化时需要从新版本创建候选并替代旧记忆。
