...
- macOS 钉钉本地文件变化可以作为消息活动信号。
- DWS 可以读取指定联系人消息并以消息 ID 去重。
- 本地规则可以判断部分“不回复”场景。
- 可将复杂消息写入 JSONL Codex 队列。

当前限制：

- 只支持一个硬编码联系人。
- 监听进程未安装为常驻服务。
- 本地文件事件是钉钉私有实现细节，可能随客户端升级失效。
- JSONL 不是事务队列，追加实现会反复重写整个文件。
- 消息规则只看单条文本，没有安静窗口和会话状态。
- Codex 队列没有消费者；CLI 冷启动曾超时。
- 没有能力网关、审批、正式记忆、发送闭环和审计数据库。
...
Production 使用 PostgreSQL；单机 MVP 可先用 SQLite，但表结构保持兼容。
...
- MVP 可使用 PostgreSQL `FOR UPDATE SKIP LOCKED` 任务队列。
- 每个会话使用串行分区，防止乱序回复。
- 不同会话和只读任务可并发。
- 同一仓库写任务需要仓库级租约。
- 生产发布需要环境级互斥锁。
...
- macOS `LaunchAgent`：
  - `ai-employee-ingestion`
  - `ai-employee-worker`
  - `ai-employee-admin`
- SQLite 或本地 PostgreSQL。
- macOS Keychain 保存 DWS/Codex 运行凭证引用。
- 日志写入受限目录并轮转。
- 健康检查和菜单栏/本地页提供暂停开关。
...
- SQLite/PostgreSQL、消息表、游标、事务 Outbox。
- 多联系人配置。
- 安静窗口、合并、去重和恢复。
- LaunchAgent 与健康检查。
...
1. 保留 DWS 读取和本地活动信号验证代码，改造成适配器。
2. 删除硬编码联系人，改为数据库授权配置。
3. 用数据库消息表和唯一约束替换 `state.json`。
4. 用事务 Outbox/任务表替换 JSONL 队列。
5. 将 `classifyMessage` 移入 Conversation 领域服务。
6. 新增能力网关后，才实现 DWS 自动发送。
7. Codex Worker 独立进程运行，不再由监听器内联启动。
8. 完成影子模式指标后再开放自动回复。
