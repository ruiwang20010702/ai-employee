...
一键安装器先只读核对 macOS、Node.js 22.5+、系统 Git 和 `uv`，再按固定顺序执行上游、补丁和发行层三个幂等步骤。全部内容只写 `.runtime/hermes-poc`；发行层失败会恢复旧组件，不会自动启动 Gateway、复制凭据、发送消息或切换 active。排障时仍可分别运行 `hermes:prepare`、`hermes:patch` 和 `hermes:install`。

### 12.2 生产迁移原则

当前生产仍运行旧 Node.js 服务并保持唯一发送权。Hermes shadow 已完成第一阶段常驻：

- runtime、OAuth、插件和状态位于独立 Application Support 版本，不从 `~/Documents` 工作树启动；
- launchd 启动受信 Node supervisor，Node 再启动 venv Python Gateway；
- Hermes PID 必须是 launchd Node PID 的直接子进程；
- `send=false`，生产 shadow 注册表全部 `read-only`；未路由 fallback 位于所有项目之外；
- DWS 首检并发执行，只有权限 `600` 的全量成功检查点才算健康；
- launchd 重启后复用去重、收件人与路由状态，检查点继续推进。

第二阶段先由 `src/hermes-shadow-acceptance.mjs` 和 `scripts/生成Hermes影子验收.mjs` 从脱敏账本生成凭据，再由 `src/hermes-cutover.mjs` 和 `scripts/切换Hermes生产运行时.mjs` 执行切换。`src/hermes-release-identity.mjs` 逐文件绑定精确提交、三个上游补丁、桥接器、边界插件和两份注册表，任何内容漂移或符号链接替换都会在停旧服务前拒绝。活动消息、任务、未知发送、活动/失败计划和过期租约必须为零；并要求 7 天内绑定精确提交的十项 shadow 验收，覆盖白名单消息、项目路由、个人记忆、自然回复、追问、代码工作、接管、重启、无发送和无重复。随后按“停旧写入者 → 验证全部停止 → 启动 active Hermes → 回读发送开关与检查点 → 写切换回执”执行。失败顺序固定为“先停 Hermes → 恢复旧写入者 → 验证旧服务”，不能并行恢复；成功后的显式 rollback 也遵循同一顺序，并恢复 `send=false` shadow。
