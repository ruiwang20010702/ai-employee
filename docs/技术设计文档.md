...
- SQLite 消息唯一键、任务租约、失败重试、死信和副作用账本。
...
- 监听进程尚未安装为 LaunchAgent 常驻服务。
- 本地文件事件是钉钉私有实现细节，可能随客户端升级失效；失效时会退化到定时检查。
- SQLite 敏感内容使用 AES-256-GCM 字段级加密；正式环境尚需接入独立密钥管理。
- 当前能力网关只开放草稿和单聊发送，没有开放方案、代码、共享文档或部署执行。
- 尚未实现正式记忆、管理页面、多租户、指标告警和生产恢复演练。
...
3. SQLite 消息表和唯一约束已经替换 `state.json`。
